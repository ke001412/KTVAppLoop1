...
public class Copy_3_of_HomeGeMingFragment extends BaseFragment implements
...
//	public static String key="";
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = null;
		try {
			view = inflater.inflate(R.layout.copygeming_layout, container, false);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//			}
	...
	static final Copy_3_of_HomeGeMingFragment INSTANCE = new Copy_3_of_HomeGeMingFragment();

	public static Copy_3_of_HomeGeMingFragment getInstance() {
		return Copy_3_of_HomeGeMingFragment.INSTANCE;
	}
	...
	private void initDataByShouPin(final String shoupin) {
		new Thread(){
			public void run() {
				...
				SongPage songPage = SongUtil.getInstance().getDefaultSongList(DEFAULT_INIT_SIZE);//SongUtil.getInstance().getSongPage(1);
				Message msg = myHandler.obtainMessage();
				msg.obj = songPage;
...
//		page = new PageSongUtil(PAGE_SIZE, freshList);
	...
	//最后初始化的页数
 	int defaultLast = DEFAULT_INIT_SIZE/2;
 	//page的位置  默认是在3
 	...
 	private void method1()
 	{
		...
		int nextPageNum = defaultLast+1;
		 
		if(nextPageFlag)
		{
			viewPageNum = viewPageNum +1;
			if(viewPageNum>=DEFAULT_INIT_SIZE)
			{
				viewPageNum=0;
			}
		}else 
		{
			...
			if(viewPageNum<0)
			{
				viewPageNum=DEFAULT_INIT_SIZE-1;
			}
		}
		if(nextPageNum > allPage)
		{
			nextPageNum = allPage;
		}
		if(nextPageNum <=0)
		...
		List<Song> listSong = SongUtil.getInstance().getCurrentSongPage(nextPageNum);
		Message msgs = new Message();
		msgs.obj = listSong;
		msgs.what = MSG_FRESH;
		myHandler.sendMessage(msgs);
		defaultLast = nextPageNum;
...
// 		}
 		if(prePageFlag)
 		{
 			...
 			List<Song> listSong = SongUtil.getInstance().getCurrentSongPage(tempPage);
	 		Message msgs = new Message();
			msgs.obj = listSong;
			msgs.what = MSG_FRESH;
			myHandler.sendMessage(msgs);
	...
	OnPageChangeListener pageChangeListener = new OnPageChangeListener() {
		...
		@Override
		public void onPageSelected(final int position,boolean prePage,boolean nextPage) {
...
//			 nextSongThread.start();
	...
	private void initView(View view) {
		...
		cycleViewPager = (CycleViewPager)view.findViewById(R.id.geming_scroll_layout);
		
		  for(int i=0;i<DEFAULT_INIT_SIZE;i++)
		  {
//			  SongPage song = SongUtil.getInstance().getSongPage(i+1);
			  View view1 = LayoutInflater.from(getActivity()).inflate(R.layout.page_item, null);
//			  initViwePage(song.getCurrentPageList(),view);
			  pages.add(view1);
...
//			initViwePage(list, view);
				List<Song>list  = (List<Song>)msg.obj;
//				 
				addNewView(list,nextPageAddNum);
...
//				SongPage songPage = SongUtil.getInstance().getSongPage(1);
				SongPage songPage = SongUtil.getInstance().getDefaultSongList(DEFAULT_INIT_SIZE);
	...
	{
		List<List<Song>> defaultList = songPage.getDefaultSongList();
		
//		View view0 = pages.get(0);
//		initViwePage(defaultList.get(0),view0);
//		View view1 = pages.get(1);
//		initViwePage(defaultList.get(1),view1);
//		View view2 = pages.get(2);
//		initViwePage(defaultList.get(2),view2);
// 
//		View view3 = pages.get(3);
//		initViwePage(defaultList.get(3),view3);
//		View view4 = pages.get(4);
//		initViwePage(defaultList.get(4),view4);
//		View view5 = pages.get(5);
//		initViwePage(defaultList.get(5),view5);

		for(int i = 0;i<DEFAULT_INIT_SIZE;i++)
		{
			View view0 = pages.get(i);
			initViwePage(defaultList.get(i),view0);
		}
		  pageAdapter.notifyDataSetChanged();
