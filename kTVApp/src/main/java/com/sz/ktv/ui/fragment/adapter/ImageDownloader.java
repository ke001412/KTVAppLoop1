...
public class ImageDownloader {
	 ...
	 */
	public void imageDownload(String url,ImageView mImageView,String path,OnImageDownload download){
		...
		//文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程。
			...
			MyAsyncTask task = new MyAsyncTask(url, mImageView, path,download);
			if(mImageView != null){
...
//				}
	 ...
	 * @return
	 */
	private boolean setBitmapToFile(String path,String imageName,Bitmap bitmap){
		File file = null;
		String real_path = path+"";
		try {
//			if(Util.getInstance().hasSDCard()){
...
//			}
	...
	private class MyAsyncTask extends AsyncTask<String, Void, Bitmap>{
		...
		public MyAsyncTask(String url,ImageView mImageView,String path,OnImageDownload download){
...
//					 final Drawable drawable = Drawable.createFromStream(new FileInputStream(new File(path)), "image.png");  
					InputStream bitmap_data = new FileInputStream(new File(path));//c_url.openStream();
