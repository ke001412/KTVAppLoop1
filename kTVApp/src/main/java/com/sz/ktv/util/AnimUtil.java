...
public class AnimUtil {
	 ...
	 * @param song 
	  */
    public static void addYiDianSong( final View iv, final Song song) {
    	rl = Global.homeLayout;
    	Drawable dr = new BitmapDrawable(BitmapUtil.getViewBitmap(iv));
//      一、创造出执行动画的主题---imageview
        //代码new一个imageview，图片资源是上面的imageview的图片
        // (这个图片就是执行动画的图片，从开始位置出发，经过一个抛物线（贝塞尔曲线），移动到购物车里)
   
    RelativeLayout cart = Global.yidianLayout;
    
        final ImageView goods = new ImageView(Global.currentActivity);
//        goods.setImageResource(R.drawable.add_usb);
        goods.setImageDrawable(dr);
...
//        三、正式开始计算动画开始/结束的坐标
        //开始掉落的起始点：商品起始点-父布局起始点+该商品图片的一半
...
//            	Song song = (Song)iv.getTag();
            	DataBase.getInstance().addYiDianSong(song);
